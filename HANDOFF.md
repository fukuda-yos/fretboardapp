...
| クラス | 意味 |
|--------|------|
| `.chord-tone` / `.chord-root` | コード構成音（下半分がピッチカラーのまま残る）/ ルート（下端に白ライン） |
| `.dimmed` | コード非構成音（**下半分ブラックアウト**。2026-07-19 に全体減光から変更） |
| `.scale-tone` / `.scale-root` | スケール構成音（上半分がピッチカラーのまま残る）/ ルート（上端に白ライン） |
