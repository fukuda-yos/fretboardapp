...
- **未解決フィードバック（2026-07-23、ユーザー実機）**: 自動ボイシングに「時々違和感がある」との指摘。
  具体的な症状（特定のコード進行で不自然／特定の転回形に偏る等）はユーザー自身もまだ特定できていない。
  **後続モデルへ**: 症状が具体化するまで手を入れない。ロードマップ#10（ソング機能）実装時に
  複数進行を通しで再生して検証する好機なので、そのタイミングでまとめて洗い出すこと。
  音の性格が変わる変更のため、修正する場合は本項目のとおり戻し方を必ず併記する。
...
| 5b | PWA化（iOS/Android対応） | **完了（2026-07-19、ユーザー実機確認済み）** |
| 6 | UIレイアウト改善 | **案4で実装完了**（Phase 0〜2 + 実機FB多数反映。残りは Phase 4 磨き＝任意のみ）。次は 5b PWA化 |
| 7 | カポ位置シミュレーター | **完了（2026-07-19、ユーザー確認済み。番号2段表示の指摘反映済み）** |
| 8 | BPMシンク | 仕様確定済み（下記）。着手中 |
| 9 | トランスポーズ機能 | キー全体を半音単位でシフト |
| 10 | コード進行ビルダー + アウフタクト対応 + ソング機能 + コードメモ | ユーザー提案(2026-07-23)を統合。コード進行を組んで最大4小節分リズムに同期させて再生する「ソング機能」＋選んだコード/進行にテキストメモを残せる機能。ボイスリーディング基盤(修正④)は先行実装済みだが、下記の違和感フィードバックを先に検証すること |
| 11 | カポ込みコードフォーム逆引き | |
| 12 | オープンチューニング対応 | INSTRUMENTS定義の拡張で対応可 |
| 13 | 鍵盤モード | 保留（優先度低） |
| 14 | カスタムリズムパターン機能 | ユーザー提案(2026-07-23)。既存の`RHYTHM_PATTERNS`/`metroScheduler`を拡張し、拍ごとのON/OFF・アクセントを編集してlocalStorageに保存する形を想定。低リスク。**UI参考: audiothing SR-88のシーケンス部分（ユーザー指定）** |
| 15 | チューナー機能 | ユーザー提案(2026-07-23)。`AnalyserNode`+自己相関法でピッチ検出（外部ライブラリ不要）。マイク許可とiOS実機確認が必要。既存`INSTRUMENTS`の開放弦定義と連携 |
...
manifest.webmanifest  ← PWA マニフェスト（2026-07-19 追加）
sw.js                 ← Service Worker（キャッシュ戦略はファイル冒頭コメント参照）
icons/                ← PWA アイコン（icon-192/512.png, apple-touch-icon.png）
...
### 2026-07-23 BPMシンク（ロードマップ #8）
- `#arpSpeed`（試聴アルペジオ用）と`#metroSpeed`（メトロノーム用）の2本のBPMスライダーを
  `#globalBpm`（range 40–300）1本に統合。試聴パネル・メトロノームパネルから撤去し、
  ツールバー右端（`#selOnlyBtn`の隣）に常時表示する`.listen-speed`ブロックとして新設
- コード変更点:
  1. `bpmToMs()` の参照先を `document.getElementById('globalBpm')` へ
  2. メトロノーム側の変数名を `metroSpeedInput`/`metroSpeedVal` → `globalBpmInput`/`globalBpmVal` に統一し、
     `metroScheduler()` 内の参照・inputイベントリスナー・リズムパターン選択時の`defaultBpm`適用先を
     すべてこちらへ差し替え
  3. 旧`#arpSpeed`用のinputリスナー（表示更新のみ）は不要になったため削除
- メトロノームのBPM上限が240→300に拡大（クリック音は短いため問題なし、仕様通り）
- ボイシング・発音アルゴリズム自体は無変更。スケジューラは毎tick BPMを読み直す設計のため、
  再生中にスライダーを動かしても即座に反映される（既存の設計どおり）

#### 回帰テスト表への追加項目（上記に伴う）
| # | 操作 | 期待 |
|---|------|------|
| 22 | ツールバー右端のBPMスライダーを操作 | 試聴（アルペジオ）とメトロノーム/リズム再生の両方に即反映。範囲40〜300 |
| 23 | リズムパターンを選択（例: ロカビリー） | ツールバー右端のBPM表示がdefaultBpm（170）に更新される |

### 2026-07-19 カポ位置シミュレーター（ロードマップ #7）
- 楽器・音色ドロワーに「カポ」セレクタ新設（`#capoSelect`、なし/1〜9フレット）
- 方式は**「カポ=新しいナット」**: OP列がカポ位置の音（`openMidi+capo`）になり、
  フレット列はカポの次から**物理フレット番号のまま**表示（ドット位置も物理準拠）。
  グリッド列数は `fretCount-capo` に減る
- 実装: 新グローバル `capoFret`（デフォルト0）を `buildFretboard` が参照。
  ループ開始を `CAPO+1` にしただけで、セルの midi は従来どおり `openMidi+f`（物理）なので
  コード/スケールハイライト・同音ハイライト・選択音のみモードは再構築で自動追従
- **フレット番号はカポ基準の2段表示**（2026-07-19 ユーザー指定で刷新）:
  **内側（盤面側）=カポ基準 0〜（主・明るめ `.fn-rel`）/ 外側=物理番号（従・暗め `.fn-phys`）**。
  つまり上の番号行は「物理↑/カポ基準↓」、下の番号行は「カポ基準↑/物理↓」（ユーザー指定の並び）。
  カポなしは従来の1段のまま、ただし色はカポ基準側と同じ明るさ（`--text-mid`）に統一
- playingBar もカポ基準: 開放列は常に「Open」、フレットセルは「1 (実4F)」形式
  （fret 引数に表示用文字列を渡すだけ。handleCellClick 自体は無変更）
- 視覚: カポ使用中の開放列に琥珀色の左バー（`.capoed`）。サブタイトルに
  `Capo 3F → (G C F A# D G)` 形式で実効チューニングを表示
- 試聴ボイシングは抽象ボイシング（盤面非依存）のためカポの影響を受けない（正しい挙動）
- 楽器切替時もカポ設定は維持（ベースにも適用可能）

#### 回帰テスト表への追加項目（上記に伴う）
| # | 操作 | 期待 |
|---|------|------|
| 21 | カポ3を選択 | OP列がG C F A# D G（ギター）。フレット番号は2段で**内側=カポ基準0〜21（明）/外側=物理3F〜24（暗）**、ドットは物理5/7/9…、開放列に琥珀マーク。タップで「フレット 1 (実4F)」形式、開放は「Open」。コードハイライトは移動後の実音に付く。カポなしに戻すと1段表示（明るい色）に戻る |

### 2026-07-19 PWA化（ロードマップ 5b）
- 追加: `manifest.webmanifest` / `sw.js` / `icons/`（192・512・apple-touch-icon 180px。
  アイコンはフレットボード+ピッチカラー4色玉のオリジナル。SVG→Edgeヘッドレスで生成）
- index.html への追記は head 3行（manifest / theme-color / apple-touch-icon）と
  INIT 直前の SW 登録ガード付き4行のみ。**file:// 直開きでは SW 登録せず従来どおり動く**
- キャッシュ戦略（仕様から1点変更あり）:
  - index.html（ナビゲーション）= **ネットワーク優先 + 失敗時キャッシュ**。
    仕様は「キャッシュファースト」だったが、受け入れ基準④「push したら次回起動時に
    新版へ切り替わる」を版番号の手動バンプなしで満たすため変更（オフライン時は前回版で起動）。
    → **index.html だけの変更なら sw.js のバージョンを上げる必要はない**
  - アイコン/manifest = キャッシュファースト。**これらを変更したら `sw.js` の
    `CACHE_NAME`（現: fpm-v1）を上げること**（activate で旧キャッシュ自動削除）
  - Google Fonts = ネットワーク優先 + 成功時キャッシュ + 失敗許容
- 検証済み（ローカルHTTPサーバー + Edge ヘッドレス）: SW 登録 → `fpm-v1` に6件プリキャッシュ
  → サーバー停止（=オフライン）状態で index.html がキャッシュから配信され盤面 DOM も生成される
- **実機確認済み（2026-07-19 ユーザー確認、「動作しました」）**。
  注: iOS サイレントスイッチでの消音は直せない仕様（必要ならユーザーに案内）

### 2026-07-19 バグ修正: 24F押下で一瞬スクロールバーが出る（ユーザー報告）
- 原因: 押下時 `.ringing` / ホバーの `transform:scale()` で最右端セルが
  `.board-wrapper`（overflow-x:auto）から右へ約2.8pxはみ出し、横スクロールバーが出現
- 修正: `.fretboard` に `padding-right:5px` を追加して拡大分を吸収（CSSのみ1行）。
  ヘッドレス実測: 修正前 +2.76px はみ出し → 修正後 −2.27px（内側）

### 2026-07-19 UI改善 案4 Phase 2: トップバー（ステータス表示）
- **ヘッダーを左右2分割のトップバー化**: 左=タイトル群（`.header-titles` で包んだだけ。
  id 無変更）、右=ステータス（新設 `.header-status`）。狭幅では flex-wrap で自然に縦積み。
  コンパクト化のため h1 を clamp(18px,3vw,24px) に縮小
- **`#playingBar` を `.controls-bar` からヘッダー右側へ移動**（id 維持・一字一句そのまま。
  抜けた `.controls-bar` は space-between → flex-start に変更）
- **選択中コードチップ `#chordChip` 新設**: `selectChord()` 末尾に textContent 更新1行を追記。
  空文字時は CSS `:empty` で非表示（クリア時に自動で消える）
- **リズム再生中チップ `#metroChip` 新設**: `startMetronome()`/`stopMetronome()` に
  classList 付替え1行ずつ追記。ON 時は緑チップ + ゆっくり点滅（`metroPulse`）
- JS 追記は上記3関数の計3行のみ。既存 id・関数名・クラス名は無変更
- Edge ヘッドレスで PC(1280px)/スマホ(390px) のスクリーンショット確認済み
- **これで UI案4 の必須フェーズ（Phase 0〜3 相当）は完了**。残るは Phase 4（磨き・任意）のみ

#### 回帰テスト表への追加項目（上記に伴う）
| # | 操作 | 期待 |
|---|------|------|
| 20 | コード選択→クリア / メトロ開始→停止 | ヘッダー右にコードチップ表示→消える / 緑の「リズム再生中」チップ点滅→消える。セルクリックの音名もヘッダー右に出る |
