...
| 5b | PWA化（iOS/Android対応） | **実装済み（2026-07-19）**。実機での受け入れ確認（機内モード・ホーム画面追加・iOS復帰時の音）待ち |
...
- **実機確認待ち**: ①機内モードで回帰テスト ②ホーム画面追加からフルスクリーン起動
  ③Android Chrome インストール可否 ④iOS バックグラウンド復帰後の発音（getCtx の resume で
  吸収想定）⑤iOS サイレントスイッチ消音は直せない仕様（ユーザー案内のみ）
