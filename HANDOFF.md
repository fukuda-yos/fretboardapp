...
- 開放列タップ時の playingBar は「フレット 3」のように**カポ位置の物理フレット**を表示
  （fret 引数に `CAPO` を渡すだけ。handleCellClick 自体は無変更）
...
| # | 操作 | 期待 |
|---|------|------|
| 21 | カポ3を選択 | OP列が「3F」でG C F A# D G（ギター）、フレット番号4〜、ドットは物理5/7/9…、開放列に琥珀マーク。コードハイライトは移動後の実音に付く。カポなしに戻すと完全に元どおり |
