...
- **フレット番号はカポ基準の2段表示**（2026-07-19 ユーザー指定で刷新）:
  上段=カポ基準 0〜（主・明るめ `.fn-rel`）/ 下段=物理番号（従・暗め `.fn-phys`）。
  OP列は「0 / 3F」。カポなしは従来の1段のまま
...
| # | 操作 | 期待 |
|---|------|------|
| 21 | カポ3を選択 | OP列がG C F A# D G（ギター）。フレット番号は2段（上=カポ基準0〜21/下=物理3F〜24）、ドットは物理5/7/9…、開放列に琥珀マーク。タップで「フレット 1 (実4F)」形式、開放は「Open」。コードハイライトは移動後の実音に付く。カポなしに戻すと完全に元どおり（1段表示） |
